...
class LotsSummary(QWidget):
    ...
    def __init__(self):
        ...
        self.table.setHorizontalHeaderLabels([
            ...
            "Nb JPG", "NbXT 1", "NbXT 2",
            "NbXT 3", "NbXT 4", "NbPDF (plis)", "NbPages (total)"
        ])
        self.table.setSortingEnabled(True)
        self.table.cellDoubleClicked.connect(self.open_lot_viewer)
        self.layout.addWidget(self.table)

        # Chargement automatique via config.ini
        if self.config and os.path.exists(self.config.get("lots_path", "")):
            ...
            print("⚠️ Aucun chemin valide trouvé dans config.ini.")
    ...
    # ----------------------------------------------------------
    # 🔍 Récupère numaxa + service depuis le XML type 1
    # ----------------------------------------------------------
    def get_xml_type1_infos(self, folder):
        """Retourne (numaxa, service) du premier XML de type 1 trouvé dans le lot"""
        for file in os.listdir(folder):
            if file.lower().endswith(".xml"):
                ...
                try:
                    ...
                    t = root.findtext("type", "").strip()
                    if t == "1":  # C'est bien un XML de type 1

                        meta = root.find("meta")
                        if meta is not None:
                            numaxa = meta.findtext("numaxa", "").strip()
                            service = meta.findtext("service", "").strip()
                        else:
                            numaxa = ""
                            service = ""

                        return numaxa, service

                except Exception as e:
                    print("Erreur XML :", path, e)
    ...
    def load_summary(self, lots_dir):
        ...
        for subdir in sorted(os.listdir(lots_dir)):
            ...
            if os.path.isdir(subpath) and subdir.isdigit() and len(subdir) == 5:
                ...
                # 🔍 Lecture XML type=1
                numaxa, service = self.get_xml_type1_infos(subpath)
                ...
                for col, val in enumerate(values):
                    item = QTableWidgetItem(val)
                    # if col != 2:  # tout centré sauf N° Lot
                    if col not in (0,1,2):  # tout centré sauf N° Lot
                        item.setTextAlignment(Qt.AlignCenter)
    ...
    def open_lot_viewer(self, row, column):
        ...
        lot_name = self.table.item(row, 2).text()  # colonne N° Lot déplacée
