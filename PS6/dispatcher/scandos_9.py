...
class LotViewer(QWidget):
    ...
    def rotate_image(self):
        """Effectue une rotation de 90° de l'image affichée"""
        if not self.scene.items():
            return  # Pas d'image à tourner

        # Récupère l'image actuelle
        item = self.scene.items()[0]  # On suppose qu'il n'y a qu'une seule image dans la scène
        pixmap = item.pixmap()

        # Crée une nouvelle pixmap tournée de 90°
    ...
    def save_rotated_image(self):
        """Enregistre la pixmap tournée dans un fichier"""
        if not self.scene.items():
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            ...
            os.path.join(self.lot_path, f"rotated_{os.path.basename(self.image_items[self.current_index][0])}"),
            "Images (*.jpg *.png)"
        )

        if file_path:
            # Enregistre la pixmap dans le fichier
