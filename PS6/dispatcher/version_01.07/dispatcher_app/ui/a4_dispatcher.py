...
class LotViewer(QWidget):
    ...
    def __init__(self, lot_path, parent=None, config=None):
        ...
        layout2.addWidget(self.result_list)
    ...
    def do_search1(self):
        ...
        model = db.fulltext_search_complexe(text,50)
        self.view.setModel(model)


    def do_search(self):
        text = self.search_input.text()
        results = db.fulltext_search_complexe_list(text, 50)
    ...
    def on_item_selected1(self, item):
        # 🎯 Action future ici
    ...
    def on_item_selected(self, item):
        ...
        value = item.text()
        print("Vous avez cliqué sur :", value)

        # tu peux appeler une autre fonction avec cette valeur
        self.traiter_complexe(value)    


    def traiter_complexe(self, value):
        # ici tu peux faire ce que tu veux avec la ligne sélectionnée
        QMessageBox.information(self, "Sélection", f"Complexe sélectionné : {value}")
